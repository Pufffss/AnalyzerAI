...
import asyncio
import os
from config.ollama_model_client import get_model_client
from config.docker_utils import getDockerCommandLineExecutor, start_docker_container, stop_docker_container
from team.analyzergpt import getDataAnalyzerTeam
from autogen_agentchat.messages import TextMessage
from autogen_agentchat.base import TaskResult

st.title('AnalyzerAI - Digital Data Analyzer')
...
# task = st.text_input("Enter your task", value='Can you give me the number of columns in my data (file is data.csv)')

task = st.chat_input("Enter your Task.")

async def run_analyzer(docker, ollama_model_client, task):
...
if task:
    ...
    ollama_model_client = get_model_client()
    docker = getDockerCommandLineExecutor()

    error = asyncio.run(run_analyzer(docker, ollama_model_client, task))
